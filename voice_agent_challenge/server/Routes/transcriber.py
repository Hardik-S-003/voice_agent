...
# expects ASSEMBLY_AI_API_KEY in env
    ...
    def __init__(
        ...
        self.client.connect(
            StreamingParameters(
                ...
                # We want raw turn text as it comes;
                # we'll ask to format once we hit end_of_turn below.
                format_turns=False,
            )
        )
        ...
        if event.end_of_turn:
            ...
            if self.on_final_callback:
                try:
                    ...
                    print("on_final_callback error:", cb_err)

            # request formatted turns for subsequent output (optional)
            ...
            # notify interim partial
            if self.on_partial_callback:
                try:
                    self.on_partial_callback(text)
                except Exception as cb_err:
                    print("on_partial_callback error:", cb_err)
